<template>
  <v-layout>
    <v-flex xs12>
      <v-flex v-if="posts.length==0" text-xs-center display-1 py-5>User didnt have any posts yet</v-flex>
      <v-flex>
...
export default {
  ...
  methods: {
    ...
    getPosts() {
      ...
      this.$http
        .get(
          `http://localhost/binzo/backend/apis/user/getUser.php?id=${id}&page=${this.posts_page}`,
          {
            headers: {
